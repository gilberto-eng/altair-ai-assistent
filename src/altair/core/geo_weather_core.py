...
def _geocodificar_cidade(cidade: str) -> Tuple[Optional[Dict], Optional[str]]:
    ...
    if not nome:
        return None, "Cidade não informada."
    ...
    try:
        ...
        return None, f"Erro ao consultar localização: {e}"

    if resp.status_code != 200:
        return None, f"Falha ao consultar localização (HTTP {resp.status_code})."

    try:
        dados = resp.json()
    except Exception:
        return None, "Falha ao interpretar resposta de localização."

    if not dados:
        return None, f"Não encontrei a cidade '{nome}'."
    ...
    try:
        ...
        return None, f"Não consegui coordenadas para '{nome}'."
...
def obter_clima_cidade_formatado(cidade: str) -> Tuple[str, str]:
    ...
    desc = WEATHER_CODE_DESC.get(codigo, "condição meteorológica não identificada")
    ...
    texto_visual = (
        ...
        f"Temperatura {temp_visual}°C, sensação {sens_visual}°C, "
        f"umidade {umid_visual}%, precipitação {chuva_visual} mm, "
        f"vento {vento_visual} km/h."
    )
    texto_fala = (
        f"Clima em {geo['nome']}, agora: {desc}. "
        f"Temperatura de {temp_fala} graus Celsius, sensação de {sens_fala} graus Celsius, "
        f"umidade de {umid_fala} por cento, precipitação de {chuva_fala} milímetros, "
        f"e vento de {vento_fala} quilômetros por hora."
    )
...
def calcular_distancia_entre_cidades(cidade_a: str, cidade_b: str, abrir_mapa: bool = True) -> str:
    ...
    return (
        f"Distância aproximada entre {origem['nome']} e {destino['nome']}: {km:.1f} km. "
        f"{msg_mapa}"
    )
