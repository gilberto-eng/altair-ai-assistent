...
def formatar_resultado_matematico(resultado: Any, modo_bonito: bool = MODO_RESPOSTA_BONITA_PADRAO) -> str:
    if not modo_bonito:
...
def resolver_equacao_direta(texto: str) -> Optional[Union[float, List[sp.Expr]]]:
    if not texto:
        return None
    x = sp.symbols("x")
    try:
...
def formatar_numero_para_fala(numero: Union[float, int]) -> str:
...
def resolver_segundo_grau(comando: str) -> Optional[List[sp.Expr]]:
    x = sp.symbols("x")
    try:
...
def maximo_minimo(comando: str) -> List[Tuple[sp.Expr, sp.Expr]]:
    x = sp.symbols("x")
...
def derivada_avancada(comando: str) -> sp.Expr:
    x = sp.symbols("x")
...
def integral_avancada(comando: str) -> sp.Expr:
    x = sp.symbols("x")
...
def integral_definida(comando: str) -> Optional[float]:
    x = sp.symbols("x")
    comando = _normalizar_texto(comando)
...
def resolver_sistema(eq1: str, eq2: str) -> Optional[List[Dict[str, Any]]]:
    x, y = sp.symbols("x y")
    try:
...
def formatar_expressao_para_fala(expr: Any) -> str:
    expr = str(expr)
...
ALIAS_FORMULAS: Dict[str, List[str]] = {
...
def buscar_formula(consulta: str) -> Optional[Tuple[str, Union[str, Dict[str, str]]]]:
...
def _extrair_intervalo(comando: str) -> Tuple[float, float]:
    texto = _normalizar_texto(comando)
...
def _extrair_funcao_grafico(comando: str) -> Optional[str]:
    original = comando or ""
...
def gerar_grafico_funcao(comando: str, base_dir: str) -> str:
    ...
    if x0 == x1:
        x0, x1 = -10.0, 10.0
    xs: npt.NDArray[np.float64] = np.linspace(x0, x1, 400)
    try:
        ys: npt.NDArray[np.float64] = f(xs)
    except Exception:
...
def simular_lancamento(comando: str, base_dir: str) -> str:
    ...
    ts: npt.NDArray[np.float64] = np.linspace(0, max(t_total, 0.1), 300)
    xs: npt.NDArray[np.float64] = vx * ts
    ys: npt.NDArray[np.float64] = h0 + vy * ts - 0.5 * g * ts**2
    ys = np.maximum(ys, 0)
...
def simular_osc_harmonica(comando: str, base_dir: str) -> str:
    ...
    ts: npt.NDArray[np.float64] = np.linspace(0, dur, 400)
    xs: npt.NDArray[np.float64] = A * np.cos(w * ts + fase)
