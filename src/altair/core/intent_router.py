...
def resolver_intencao_comando(comando: str) -> Tuple[Optional[str], Dict[str, str]]:
    ...
    comando_original = re.sub(r"^\s*debug\s*:\s*comando\s*recebido\s*->\s*", "", comando_original, flags=re.IGNORECASE).strip()
    ...
    if re.search(r"\b(grafico|grÃ¡fico)\b", comando_original, re.IGNORECASE):
        return "gerar_grafico", {"comando": comando_original}

    if re.search(r"\b(simular|simulacao|simulaÃ§Ã£o)\b", comando_original, re.IGNORECASE):
        return "simular_fisica", {"comando": comando_original}

    if re.search(
        r"\b(cad|cadquery|modelo 3d|projeto 3d|modelar|modelagem 3d|3d|cubo|cilindro|esfera|furo|suporte|placa|engrenagem|engrenagens|gear|gears|eixo|shaft)\b",
        comando_original,
        re.IGNORECASE,
    ):
        return "gerar_cad_3d", {"comando": comando_original}

    if re.search(r"\b(formula|fÃ³rmula|formulas|fÃ³rmulas|equacao|equaÃ§Ã£o)\b", comando_original, re.IGNORECASE):
    ...
    match_audio = re.search(
        r"(?:enviar|envie|mande|encaminhe)\s+(?:audio|Ã¡udio)\s+(?:para|pra|pro)\s+(.+?)\s+(?:dizendo|falando|com a mensagem|mensagem)\s+(.+)$",
        comando_original,
        re.IGNORECASE,
    )
    if match_audio:
    ...
    match_clima = re.search(
        r"(?:\bclima\b|\btempo\b|\bprevis(?:ao|ão)\b).{0,20}\b(?:em|de|para)\b\s+(.+)$",
        comando_original,
        re.IGNORECASE,
    )
    if match_clima:
    ...
    match_distancia = re.search(
        r"(?:\bdistancia\b|\bdistância\b)\s+(?:entre|de)\s+(.+?)\s+(?:e|para|ate|até)\s+(.+?)(?:\s+(?:no|na)\s+mapa)?$",
        comando_original,
        re.IGNORECASE,
    )
    if match_distancia:
...
def executar_intencao(
    intencao: str, dados: Dict[str, str], contexto: Dict[str, Any]
) -> Dict[str, str]:
    ...
    if intencao == "calcular_na_calculadora":
        ...
        return {
            "visual": f"Calculadora aberta e operaÃ§Ã£o executada: {expr} = {resultado_num}",
            "fala": f"O resultado Ã© {resultado_num}",
        }
    ...
    if intencao == "gerar_grafico":
        ...
        if isinstance(resp, str):
            m = re.search(r"(Grafico|GrÃ¡fico).*?:\s*(.*\.png)", resp, re.IGNORECASE)
            if m:
                caminho = m.group(2).strip()
        if caminho and os.path.exists(caminho):
            return {"visual": f"GrÃ¡fico gerado: {caminho}", "fala": "", "file": caminho}
    ...
    if intencao == "simular_fisica":
        ...
        if "lancamento" in cmd_norm or "lanÃ§amento" in cmd_norm or "projetil" in cmd_norm:
            ...
            return payload
        if "oscil" in cmd_norm or "pendulo" in cmd_norm or "pÃªndulo" in cmd_norm:
    ...
    if intencao == "consultar_formula":
        ...
        if not resultado:
            return {"visual": "Nao encontrei essa formula. PeÃ§a por 'lista de formulas' para ver o catalogo.", "fala": ""}
